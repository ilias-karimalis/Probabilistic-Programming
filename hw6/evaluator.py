...
def run_deterministic_tests(use_cache=True, cache='programs/tests/'):
    ...
    for i in range(1, 13):
        if use_cache:
            ...
            exp = daphne(
                ['desugar-hoppl-cps', '-i', '../../HW6/programs/tests/hoppl-deterministic/test_{}.daphne'.format(i)])
...
if __name__ == '__main__':
    ...
    res = evaluate(exp, env=None)('addr_start',
                                  output)  # set up the initial call, every evaluate returns a continuation, a set of arguments, and a map sigma at every procedure call, every sample, and every observe
    cont, args, sigma = res
